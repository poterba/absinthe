...
int randomize_string(unsigned char* buffer, unsigned int size, int times)
{
    ...
    for (i = 0; i < times; i++) {
        ...
        // debug("Index = 0x%02x", index);
        // debug("Original byte found was 0x%x", buffer[index]);
        ...
        // debug("New byte is 0x%x", buffer[index]);
        buffer[index] &= ~0x80;
    }
    return 0;
}
...
/*
        ...
        if(buffer) {
                ...
                for(i = 0; i < count; i++) {
                        ...
                        //debug("Bit = %d", bit);
                        int index = bit/8; // returns the byte index of the bit we're going to
change
                        //debug("Index = 0x%02x", index);
...
target[i+2] = addr[2];
target[i+3] = addr[3];
debug("ht->callbacks = 0x%08x", ht->callbacks);
}
...
printf("ht->bits.hash_style = %d", ht->bits.hash_style);
printf("ht->bits.values2_offset = %d", ht->bits.values2_offset);
printf("ht->bits.keys_offset = %d", ht->bits.keys_offset);
printf("ht->bits.keys2_offset = %d", ht->bits.keys2_offset);
printf("ht->bits.counts_offset = %d", ht->bits.counts_offset);
printf("ht->bits.orders_offset = %d", ht->bits.orders_offset);
printf("ht->bits.hashes_offset = %d", ht->bits.hashes_offset);
printf("ht->bits.num_buckets_idx = %d", ht->bits.num_buckets_idx);
printf("ht->bits.used_buckets = %d", ht->bits.used_buckets);
printf("ht->bits.finalized = %d", ht->bits.finalized);
printf("ht->bits.fast_grow = %d", ht->bits.fast_grow);
printf("ht->bits.strong_values = %d", ht->bits.strong_values);
printf("ht->bits.strong_values2 = %d", ht->bits.strong_values2);
printf("ht->bits.strong_keys = %d", ht->bits.strong_keys);
printf("ht->bits.strong_keys2 = %d", ht->bits.strong_keys2);
printf("ht->bits.marker = %d", ht->bits.marker);
printf("ht->bits.deleted = %d", ht->bits.deleted);
printf("ht->bits.mutations = %d", ht->bits.mutations);
printf("");
...
{
    printf("dict->bits.hash_style = %d", dict->bits.hash_style);
    printf("dict->bits.values2_offset = %d", dict->bits.values2_offset);
    printf("dict->bits.keys_offset = %d", dict->bits.keys_offset);
    printf("dict->bits.keys2_offset = %d", dict->bits.keys2_offset);
    printf("dict->bits.counts_offset = %d", dict->bits.counts_offset);
    printf("dict->bits.orders_offset = %d", dict->bits.orders_offset);
    printf("dict->bits.hashes_offset = %d", dict->bits.hashes_offset);
    printf("dict->bits.num_buckets_idx = %d", dict->bits.num_buckets_idx);
    printf("dict->bits.used_buckets = %d", dict->bits.used_buckets);
    printf("dict->bits.finalized = %d", dict->bits.finalized);
    printf("dict->bits.fast_grow = %d", dict->bits.fast_grow);
    printf("dict->bits.strong_values = %d", dict->bits.strong_values);
    printf("dict->bits.strong_values2 = %d", dict->bits.strong_values2);
    printf("dict->bits.strong_keys = %d", dict->bits.strong_keys);
    printf("dict->bits.strong_keys2 = %d", dict->bits.strong_keys2);
    printf("dict->bits.marker = %d", dict->bits.marker);
    printf("dict->bits.deleted = %d", dict->bits.deleted);
    printf("dict->bits.mutations = %d", dict->bits.mutations);
    printf("");
}
