...
compatibility_t compatible_devices[] = {{"N92AP", "9B176"}, {"N90AP", "9B176"}, {"N88AP", "9B176"},
                                        {"N18AP", "9B176"}, {"N88AP", "9B176"}, {"K48AP", "9B176"},

                                        {"N92AP", "9B206"}, {"N90AP", "9B206"}, {"N88AP", "9B206"},
                                        {"N18AP", "9B206"}, {"N88AP", "9B206"}, {"K48AP", "9B206"},

                                        {"N92AP", "9A405"}, {"N90AP", "9A405"}, {"N88AP", "9A405"},
                                        {"N18AP", "9A405"}, {"N88AP", "9A405"}, {"K48AP", "9A405"},

                                        {"N92AP", "9A334"}, {"N90AP", "9A334"}, {"N88AP", "9A334"},
                                        {"N18AP", "9A334"}, {"N88AP", "9A334"}, {"K48AP", "9A334"},

                                        {"N90AP", "8L1"},   {"N88AP", "8L1"},   {"N18AP", "8L1"},
                                        {"N88AP", "8L1"},   {"K48AP", "8L1"},

                                        {"N90AP", "8K2"},   {"N88AP", "8K2"},   {"N18AP", "8K2"},
                                        {"N88AP", "8K2"},   {"K48AP", "8K2"},
...
static void prefs_add_entry(plist_t* pl) /*{{{*/
{
    ...
    // UserDefinedName
    plist_dict_set_item(conn, "UserDefinedName", plist_new_string(CONNECTION_NAME));
    ...
    plist_dict_set_item(dict, "AuthenticationMethod", plist_new_string("SharedSecret"));
    ...
    plist_dict_set_item(dict, "RemoteAddress", plist_new_string("127.0.0.1"));
    plist_dict_set_item(dict, "LocalIdentifier", plist_new_string("2"));
    plist_dict_set_item(
        dict, "XAuthName",
        plist_new_string(
            "pod2g\"; proposal {authentication_method xauth_psk_client; hash_algorithm sha1; "
            "encryption_algorithm aes 256; lifetime time 3600 sec; dh_group 2;} } include "
            "\"/private/var/preferences/SystemConfiguration/com.apple.ipsec.plist"));
    plist_dict_set_item(dict, "LocalIdentifierType", plist_new_string("KeyID"));
    plist_dict_set_item(dict, "XAuthEnabled", plist_new_uint(1));
    plist_dict_set_item(dict, "SharedSecretEncryption", plist_new_string("Key"));
...
static void afc_free_dictionary(
    char** dictionary) // ghetto i know, not sure where/how to put a global function for this
{
    int i = 0;
...
static void move_back_files_afc(afc_client_t afc)
{
    ...
    while (list && list[i]) {
        ...
        debug("moving %s to %s", tmxname, tmpname);
    ...
    const char* build)
{
    ...
    status_cb("Connecting to device...", 50);
    sleep(1);
    ...
    debug("pid=%d (len=%d)", pid, pidlen);

    crashreport_free(crash);
    crash = NULL;

    debug("dscs=0x%x", dscs);
    ...
    if (ipsec_plist != NULL && ipsec_plist_size > 0) {
        ...
        if (bf) {
            ...
            backup_file::set_path(bf, "SystemConfiguration/com.apple.ipsec.plist");
...
fix_all:
    ...
    debug("Device %s detected. Connecting...", udid);
    status_cb("Device detected, connecting...", 60);
    sleep(2);
...
#define IOS_5_1_OVERRIDES_INJECT_DIR ".tmpdir2"
    ...
    debug("moving dirs aside...");
    // afc_rename_path(afc, "/Books", "/"AFCTMP"/Books"); // seems to not break it if we leave it
    // out!!!
    afc_rename_path(afc, "/DCIM", "/" AFCTMP "/DCIM");
    afc_rename_path(afc, "/PhotoData", "/" AFCTMP "/PhotoData");
    afc_rename_path(afc, "/Photos", "/" AFCTMP "/Photos");
...
#undef ADD_FILE_EXEC
    ...
    /* restore backup */
    /********************************************************/
    debug("Sending initial data. Your device will appear to be restoring a backup, this may also "
          "take a while...");
    char* rargv[] = {"idevicebackup2", "restore",  "--system", "--settings",
    ...
    char* rargv2[] = {"idevicebackup2", "restore", "--system", "--settings", backup_dir, NULL};
     ...
     */
    info("Please run the #Unthread application to remount the root filesystem as read/write. Hit a "
         "key to continue when done.");
    getchar();
...
fix:
    ...
    info("Recovery completed. If you want to retry jailbreaking, unplug your device and plug it "
         "back in. If you device has been jailbroken, reboot it.");
leave:
    afc_client_free(afc);
    afc = NULL;
    device_free(device);
