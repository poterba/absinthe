...
AbsintheWorker::AbsintheWorker(AbsintheMainWnd* main) : mainWnd(main), device_count(0)
{
    self = this;
    ...
    if (quick_check != 6) {
        this->mainWnd->setStatusText("ERROR: Could not find required files. Make sure you "
                                     "extracted the entire package!");
        return;
    }
...
char* AbsintheWorker::getUDID(void) { return current_udid; }
...
void AbsintheWorker::checkDevice()
{
    ...
    if (this->device_count == 0) {
        ...
        if (lerr == LOCKDOWN_E_PASSWORD_PROTECTED) {
            ...
            idevice_free(dev);
            sprintf(
                str, "ERROR: Device has a passcode set! If a passcode is set, the "
                     "jailbreak procedure will most likely fail. Unplug device, go to "
                     "Settings and DISABLE THE PASSCODE, then plug it back in.");
            mainwnd->setStatusText(str);
            return;
            ...
            idevice_free(dev);
            sprintf(
                str, "Device detection failed. Please unplug device and plug it back in. "
                     "Then it should work.");
            mainwnd->setStatusText(str);
            return;
            ...
            if (newl) {
                ...
                if (device_public_key && (plist_get_node_type(device_public_key) == PLIST_DATA)) {
                    ...
                    if (memcmp(testdata, chk, strlen(chk)) == 0) {
                        ...
                        idevice_free(dev);
                        sprintf(
                            str, "Device detection failed. Please unplug "
        ...
        if (cc == 0) {
            ...
            mainwnd->setStatusText(
                "ERROR: Consistency check failed: attached device not supported");
            free(productType);
            free(productVersion);
            free(buildVersion);
            lockdownd_client_free(client);
            ...
            mainwnd->setStatusText(
                "ERROR: Consistency check failed: could not find required files");
            free(productType);
            free(productVersion);
            free(buildVersion);
            lockdownd_client_free(client);
        ...
        if (node) {
            ...
            if (pcenabled) {
                sprintf(
                    str, "ERROR: Device has a passcode set! If a passcode is set, the "
                         "jailbreak procedure will most likely fail. Unplug device, go "
                         "to Settings and DISABLE THE PASSCODE, then plug it back in.");
                mainwnd->setStatusText(str);
                return;
            }
        }
        ...
        sprintf(
            str, "%s with iOS %s (%s) detected. Click the button to begin.",
            getDeviceName(productType), productVersion, buildVersion);
        mainwnd->setStatusText(str);
        ...
        if (pl && plist_get_node_type(pl) == PLIST_STRING) {
            ...
            if (as) {
                if (strcmp(as, "Unactivated") == 0) {
                    ready_to_go = 0;
                    mainwnd->msgBox(
                        "The attached device is not activated. You need to "
                        "activate it before it can be used with Absinthe.",
        ...
        if (pl && plist_get_node_type(pl) == PLIST_BOOLEAN) {
            ...
            if (c) {
                ready_to_go = 0;
                mainwnd->msgBox(
                    "The attached device has a backup password set. You need to "
                    "disable the backup password in iTunes before you can "
                    "continue.\nStart iTunes, remove the backup password and start "
                    "this Program again.",
                    "Error", mb_OK | mb_ICON_ERROR);
            }
        }
