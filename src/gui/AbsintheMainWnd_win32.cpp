...
void AbsintheMainWnd::setButtonEnabled(int enabled) { EnableWindow(this->btnStart, enabled); }
        ...
        switch (uMsg) {
            case WM_COMMAND:
                if ((HWND) lParam == lbPaypal) {
                    SetWindowPos(
                        self->mainwnd, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE);
...
AbsintheMainWnd::AbsintheMainWnd(int* pargc, char*** pargv)
{
    ...
    if (!RegisterClass(&WinClass)) {
        msgBox(
            "Error registering window class! This should not happen!", "Error",
            MB_OK | MB_ICONSTOP);
        return;
    }
    ...
    if (!RegisterClass(&PanelClass)) {
        msgBox(
            "Error registering panel class! This should not happen!", "Error", MB_OK | MB_ICONSTOP);
        return;
    }
    ...
    mainwnd = CreateWindowEx(
        WS_EX_TOPMOST, WinClass.lpszClassName, WND_TITLE,
        WS_VISIBLE | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX, xOffset, yOffset, WND_WIDTH,
        WND_HEIGHT, 0, 0, hInst, NULL);
    if (mainwnd == NULL) {
        return;
    }
    ...
    lbInfo = CreateWindowEx(
        0, "Static", tmpstr, WS_VISIBLE | WS_CHILD | SS_NOPREFIX, 10, 10, WND_WIDTH - 20, 70,
        mainwnd, (HMENU) 1, hInst, NULL);
    free(tmpstr);

    lbStatus = CreateWindowEx(
        0, "Static", "", WS_VISIBLE | WS_CHILD | SS_NOPREFIX, 10, 86, WND_WIDTH - 20, 82, mainwnd,
        (HMENU) 2, hInst, NULL);

    progressBar = CreateWindowEx(
        0, PROGRESS_CLASS, NULL, WS_CHILD | WS_VISIBLE | PBS_SMOOTH, 38, 177, 300, 17, mainwnd,
        (HMENU) 3, hInst, NULL);
    SendMessage(progressBar, PBM_SETRANGE, 0, MAKELPARAM(0, 100));
    SendMessage(progressBar, PBM_SETPOS, (WPARAM) 0, 0);

    btnStart = CreateWindowEx(
        0, "Button", "Jailbreak", WS_VISIBLE | WS_CHILD | WS_TABSTOP | WS_DISABLED, 358, 174, 75,
        23, mainwnd, (HMENU) 4, hInst, NULL);

    tmpstr = fromUTF8(CREDITS_LABEL_TEXT);
    lbCredits = CreateWindowEx(
        0, "Static", tmpstr, WS_CHILD | WS_VISIBLE | SS_CENTER | SS_NOPREFIX, 10, 210,
        WND_WIDTH - 20, 80, mainwnd, (HMENU) 5, hInst, NULL);
    free(tmpstr);

    panel = CreateWindowEx(
        WS_EX_CONTROLPARENT, PanelClass.lpszClassName, "", WS_VISIBLE | WS_CHILD, 0, 290, WND_WIDTH,
        30, mainwnd, (HMENU) 6, hInst, NULL);

    tmpstr = fromUTF8(PAYPAL_LINK_TEXT);
    lbPaypal = CreateWindowEx(
        0, "Button", tmpstr, WS_CHILD | WS_VISIBLE | BS_OWNERDRAW, 40, 4, 150, 20, panel, (HMENU) 7,
        hInst, NULL);
    free(tmpstr);

    tmpstr = fromUTF8(GP_LINK_TEXT);
    lbGP = CreateWindowEx(
        0, "Button", tmpstr, WS_CHILD | WS_VISIBLE | BS_OWNERDRAW, 280, 4, 150, 20, panel,
        (HMENU) 8, hInst, NULL);
    free(tmpstr);

    //+++ create font +++
    hFont1 = CreateFont(
        -12, 0, 0, 0, 400, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
