...
int AbsintheMainWnd::msgBox(const char* message, const char* caption, int style)
{
    ...
    GtkWidget* msgbox = gtk_message_dialog_new(
        GTK_WINDOW(this->mainwnd), GTK_DIALOG_MODAL, mtype, btype, "%s", caption);
...
void AbsintheMainWnd::setProgress(int percentage)
{
    gtk_progress_bar_set_fraction(
        GTK_PROGRESS_BAR(this->progressBar), (double) percentage / 100.0f);
...
extern "C"
{
    ...
    static void destroy_event(GtkWidget* widget, gpointer data) { gtk_main_quit(); }
}
...
AbsintheMainWnd::AbsintheMainWnd(int* pargc, char*** pargv)
{
    ...
    gchar* url =
        g_strreplace("<a href=\"" PAYPAL_LINK_URL "\">" PAYPAL_LINK_TEXT "</a>", "&", "&amp;");
