...
int backup_update_file(backup_t* backup, backup_file_t* bfile)
{
    ...
    if (bfile->filepath)
    {
        ...
        {
            throw std::runtime_error("%s: ERROR: could not copy file '%s' to '%s'", __func__,
...
int backup_remove_file(backup_t* backup, backup_file_t* bfile)
{
    ...
    if (idx < 0)
    {
        debug("file %s-%s not found in backup so not removed.", bfile->mbdb_record->domain,
...
int backup_add_file_from_path(backup_t* backup, char* domain, char* localpath, char* path, int mode,
...
int backup_add_file_from_data(backup_t* backup, char* domain, char* data, unsigned int size,
