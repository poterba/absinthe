...
mb2_t* mb2_connect(device_t* device)
{
    ...
    if (err < 0)
    {
        throw std::runtime_error("Unable to start MobileBackup2 service");
        return NULL;
    }
    ...
    if (mb2 == NULL)
    {
        throw std::runtime_error("Unable to open connection to MobileBackup2 service");
        return NULL;
    }
...
static int mb2_handle_send_file(mb2_t* mb2s, const char* backup_dir, const char* path,
                                plist_t* errplist)
{
    printf(">> %s called", __func__);
    uint32_t nlen = 0;
    ...
    printf("Sending '%s' (%ud bytes)", path, (unsigned int)total);
...
/* i dont think we ever have to receive files unless we are creating a backup, but leaving this here
 * just in case */
...
static int mb2_handle_receive_files(mb2_t* mb2s, plist_t message, const char* backup_dir)
{
    ...
    do
    {
        ...
        {
            printf("Found new flag %02x", code);
        }
...
static void mb2_handle_make_directory(mb2_t* mb2s, plist_t message, const char* backup_dir)
{
    ...
    free(newpath);
    mobilebackup2_error_t err =
        mobilebackup2_send_status_response(mb2s->client, errcode, errdesc, NULL);
    if (err != MOBILEBACKUP2_E_SUCCESS)
    {
        printf("Could not send status response, error %d", err);
    }
...
static int mb2_process_messages(mb2_t* mb2, const char* backup_directory)
{
    ...
    do
    {
        ...
        if (!message || !dlmsg)
        {
            ...
            // their old code wasn't trying again, not sure how we would either, just returning for
            // now.
            sleep(2);
            return -1;
        }
        ...
        { // int value of DLMessage
            ...
            case DLMSG_MOVE_FILES:
                ...
                printf("Moving %d file%s", cnt, (cnt == 1) ? "" : "s");
                    ...
                    do
                    {
                        ...
                        if (key && (plist_get_node_type(val) == PLIST_STRING))
                        {
                            ...
                            if (str)
                            {
                                ...
                                {
                                    printf("Renaming '%s' to '%s' failed: %s (%d)", oldpath,
            ...
            case DLMSG_REMOVE_FILES:
                ...
                printf("Removing %d file%s", cnt, (cnt == 1) ? "" : "s");
                uint32_t ii = 0;
                errdesc = NULL;
                for (ii = 0; ii < cnt; ii++)
                {
                    ...
                    if (plist_get_node_type(val) == PLIST_STRING)
                    {
                        ...
                        if (str)
                        {
                            ...
                            if (remove(newpath) < 0)
                            {
                                printf("Could not remove '%s': %s (%d)", newpath, strerror(errno),
                                       errno);
                                errcode = errno_to_device_error(errno);
                                errdesc = strerror(errno);
                            }
...
int mb2_crash(mb2_t* mb2)
{
    ...
    mobilebackup2_error_t err =
        mobilebackup2_version_exchange(mb2->client, local_versions, 2, &remote_version);
    if (err != MOBILEBACKUP2_E_SUCCESS)
    {
    ...
    printf("Negotiated Protocol Version %.1f", remote_version);
    ...
    err = mobilebackup2_send_request(mb2->client, "Restore", mb2->device->udid, mb2->device->udid,
                                     NULL);
    if (err == MOBILEBACKUP2_E_SUCCESS)
    {
    ...
    else
    {
        ...
        {
            printf("ERROR: Could not start backup process: device refused to start the backup "
                   "process.");
        }
        else
        {
...
int mb2_exploit(mb2_t* mb2) { return 0; }
